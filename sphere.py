...
# may fix the else ,elif and if

class tokening:
    def __init__(self, operator, sepertors, keyword, booleans ):
        self.operators = operator
        self.separators = sepertors
        self.keywords = keyword
        self.booleans = booleans

main = tokening(["+","-","*","/","=","^","!"],["(",")",",", ":",";"],["output", "getConsole", "if", "elif", "else", "while","turnNum","turnText","endif"], 
                ["==","!=","<",">"])

# tokenizer (tokenize)
def tokenizer(input_calculator: str):
    global main
    operators = main.operators + main.booleans
    seperators = main.separators
    token_output = []
    add_to_output = ""
    token_type = ""
    onString = False
...
def not_equals(item1, item2):
    return f"{not item1[1] == item2[1]}".lower()

def forse_negetive(num1):
    return -num1

def bigger_num(num1, num2):
    return f"{num1 > num2}".lower()
...
def finalise(step_line: list, storege: dict, if_states: list):
    ...
    keywords = main.keywords
    seperators = main.separators
    operators = main.operators + main.separators
    ...
    for i in range(len(step_line)):
        if i+1 < len(step_line):
            if roand_number_0(step_line[i-1][0]) <  roand_number_0(step_line[i][0])  and step_line[i][1][1] == '-' and step_line[i+1][1][0] == "NUMBER":
                
                if roand_number_0(step_line[i][0]) <= roand_number_0(step_line[i+1][0]):
                    result = float(step_line[i+1][1][1])
                    if result // 1 == result: 
                        result = int(result)
                    step_line[i] = (step_line[i+1][0],("NUMBER",f"{-(result)}"))
                    step_line.pop(i+1)
    ...
    while Continue:
        ...
        if step_line[location][1][0] == "KEYWORD":
            ...
            if keyword == "output":
            ...
            elif keyword == "turnNum":
                
                number = step_line[location][0] 
                minimum = roand_number_0(number)+10
                ...
                elif if_states[-1] == "true": if_states[-1]= 'false'   
                step_line.pop(location)
        ...
        # Error Adding String
            ...
            num1 = step_line[location-1][1][1]
            num2 = step_line[location+1][1][1]
            result = bigger_num(float(num1),float(num2))
            step_line[location] = (step_line[location-1][0], ('BOOL', str(result.lower())))
            step_line.pop(location+1)
            step_line.pop(location-1)
            ...
            num1 = step_line[location-1][1][1]
            num2 = step_line[location+1][1][1]
            result = lower_num(float(num1),float(num2))
            step_line[location] = (step_line[location-1][0], ('BOOL', str(result.lower())))
            step_line.pop(location+1)
            step_line.pop(location-1)
            ...
            item1 = step_line[location+1]
            result = forse_negetive(float(item1[1][1]))
            if result == int(result):
                result = int(result)
            step_line[location+1] = (step_line[location+1][0], ('NUMBER', str(result)))
            step_line.pop(location)
